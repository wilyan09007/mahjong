...
import { DISCARDS_PER_ROW, rotationFor, type Edge } from '../state/tableLayout';
...
export function DiscardPond({
  ...
  return (
    <View style={[styles.pond, { maxWidth: DISCARDS_PER_ROW * 34 }]}>
      {tiles.map((tile, i) => (
        <Tile
          key={`${tile}-${i}`}
          tile={tile}
...
export function OpponentPanel({
  ...
  return (
    <View
      style={[
        styles.opponent,
        isTurn && styles.opponentTurn,
        { transform: [{ rotate: `${rotationFor(edge)}deg` }] },
      ]}
    >
      ...
      {opponent.melds.length > 0 && <MeldGroup melds={opponent.melds} size="mini" />}
...
const styles = StyleSheet.create({
  ...
  lastDiscard: { marginTop: tokens.space.xs },
  seatCard: {
