...
| File | Plan | Key exports | What it does |
|---|---|---|---|
| `src/tiles.ts` | Task 2 | Types: `SuitCode`, `Rank`, `SuitTileKind`, `WindKind`, `DragonKind`, `HonorKind`, `FlowerKind`, `TileKind`, `Seat`, `Wind`. Consts: `SUIT_KINDS`(27), `WINDS`(4), `DRAGONS`(3), `FLOWERS`(8), `NON_FLOWER_KINDS`(34), `FULL_TILE_SET`(144). Fns: `isFlower`, `isSuitTile`, `isHonor`, `rankOf`, `suitOf`, `sortTiles`, `kindIndex`, `seatWind` | The vocabulary every other file speaks. Tile kinds, the full 144-tile set, classification/sorting helpers, and the 0–33 `kindIndex` used by win detection's count arrays. `seatWind(seat, dealer)` gives E/S/W/N relative to the dealer. |
